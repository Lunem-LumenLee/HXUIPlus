...
local function RgbTableToHex(c)
    if (c == nil) then
        return nil;
    end
    local r = math.clamp(tonumber(c[1]) or 0, 0, 255);
    local g = math.clamp(tonumber(c[2]) or 0, 0, 255);
    local b = math.clamp(tonumber(c[3]) or 0, 0, 255);
    return string.format('#%02x%02x%02x', r, g, b);
end

local function GetOorGrad(kind, fallback1, fallback2)
    local key = nil;
    if (kind == 'hp') then key = 'partyListOorHpColor';
    elseif (kind == 'mp') then key = 'partyListOorMpColor';
    elseif (kind == 'tp') then key = 'partyListOorTpColor';
    end

    local hex = nil;
    if (key ~= nil and gConfig ~= nil and gConfig[key] ~= nil) then
        hex = RgbTableToHex(gConfig[key]);
    end
    if (hex ~= nil) then
        return { hex, hex };
    end
...
local function IsPartyMemberOutOfRange(memIdx)
    local party = AshitaCore:GetMemoryManager():GetParty();
    if (party == nil) then
        return false;
    end

    local tIdx = party:GetMemberTargetIndex(memIdx);
    if (tIdx == nil or tIdx == 0) then
        return false;
    end

    local ent = GetEntity(tIdx);
    if (ent == nil or ent.Distance == nil) then
        return false;
    end

    local dist = math.sqrt(ent.Distance);
    local maxDist = tonumber(gConfig.partyListRangeDistance) or 21.8;
    return (dist > maxDist);
end



local function SubTargetByPartyMemberIndex(memIdx)
    local party = AshitaCore:GetMemoryManager():GetParty();
    if (party == nil) then
        return;
    end
...
local function DrawMember(memIdx, settings)
    ...
    local isOor = (memInfo.inzone and tostring(memInfo.name) ~= '' and IsPartyMemberOutOfRange(memIdx));

    -- Get the hp color for bars and text
    local hpNameColor, hpGradient = GetHpColors(memInfo.hpp);
    if (isOor) then
        hpNameColor = 0xFF444444;
        hpGradient = GetOorGrad('hp', '#151515', '#1f1f1f');
    end
    ...
    if (isOor) then
        memberText[memIdx].name:SetColor(0xFF555555);
    ...
    if (memInfo.inzone) then
        ...
        local mpGradient = { '#9abb5a', '#bfe07d' };
        if (isOor) then
            mpGradient = GetOorGrad('mp', '#2b2b2b', '#3a3a3a');
        end

        if (isOor) then
            mpGradient = { '#2b2b2b', '#3a3a3a' };
        end
        ...
        -- Update the mp text
        if (isOor) then
            memberText[memIdx].mp:SetColor(0xFF666666);
        else
        ...
        if (showTP) then
            ...
            if (isOor) then
                tpGradient = GetOorGrad('tp', '#2b2b2b', '#3a3a3a');
                tpOverlayGradient = GetOorGrad('tp', '#2b2b2b', '#2b2b2b');
            end
            ...
            -- Update the tp text
            if (isOor) then
                memberText[memIdx].tp:SetColor(0xFF666666);
        ...
        -- Click-to-target hitbox (covers the entire member row)
        do
            local clickX = hpStartX - settings.cursorPaddingX1;
            local clickY = hpStartY - offsetSize - settings.cursorPaddingY1;
            local clickW = allBarsLengths + settings.cursorPaddingX1 + settings.cursorPaddingX2;
            local clickH = entrySize;

            local restoreX, restoreY = imgui.GetCursorScreenPos();
            imgui.SetCursorScreenPos({ clickX, clickY });

            imgui.InvisibleButton('PartyMemberClick##' .. tostring(memIdx), { clickW, clickH });

            if (imgui.IsItemHovered()) then
                draw_rect({ clickX, clickY }, { clickX + clickW, clickY + clickH }, {1, 1, 1, 0.10}, 0, true);
            end

            if (imgui.IsItemClicked(0)) then
                if (memInfo ~= nil and memInfo.inzone) then
                    SubTargetByPartyMemberIndex(memIdx);
                end
            end
        ...
        if (partyIndex == 1 and memInfo.buffs ~= nil and #memInfo.buffs > 0) then
            if (gConfig.partyListStatusTheme == 0 or gConfig.partyListStatusTheme == 1) then
                ...
                if (buffs ~= nil and #buffs > 0) then
                    local statusRight = (gConfig.partyListStatusIconPosition == 1);

                    if (not statusRight and buffWindowX[memIdx] ~= nil) then
                        imgui.SetNextWindowPos({hpStartX - buffWindowX[memIdx] - settings.buffOffset , hpStartY - settings.iconSize*1.2});
                    elseif (statusRight and fullMenuWidth[partyIndex] ~= nil) then
                ...
                if (debuffs ~= nil and #debuffs > 0) then
                    ...
                    if (not statusRight and debuffWindowX[memIdx] ~= nil) then
...
partyList.UpdateFonts = function(settings)
    ...
    if (selectionPrim ~= nil) then
        selectionPrim.color = borderColor;
    end
