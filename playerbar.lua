...
local _HXUIPlus_DEV_DEBUG_INTERPOLATION_DELAY, _HXUIPlus_DEV_DEBUG_INTERPOLATION_NEXT_TIME;

if _HXUIPlus_DEV_DEBUG_INTERPOLATION then
	_HXUIPlus_DEV_DEBUG_INTERPOLATION_DELAY = 2;
	_HXUIPlus_DEV_DEBUG_INTERPOLATION_NEXT_TIME = os.time() + _HXUIPlus_DEV_DEBUG_INTERPOLATION_DELAY;
end
...
playerbar.DrawWindow = function(settings)
    ...
    if _HXUIPlus_DEV_DEBUG_INTERPOLATION then
	    if os.time() > _HXUIPlus_DEV_DEBUG_INTERPOLATION_NEXT_TIME then
	    	playerbar.previousHPP = 75;
	    	playerbar.currentHPP = 50;
			playerbar.lastHitTime = currentTime;

			_HXUIPlus_DEV_DEBUG_INTERPOLATION_NEXT_TIME = os.time() + 2;
	    end
	end
