...
local _HXUIPlus_DEV_DEBUG_INTERPOLATION = false;
local _HXUIPlus_DEV_DEBUG_INTERPOLATION_DELAY = 1;
local _HXUIPlus_DEV_DEBUG_HP_PERCENT_PERSISTENT = 100;
local _HXUIPlus_DEV_DAMAGE_SET_TIMES = {};
...
targetbar.DrawWindow = function(settings)
	...
	-- Mimic damage taken
	if _HXUIPlus_DEV_DEBUG_INTERPOLATION then
		if _HXUIPlus_DEV_DAMAGE_SET_TIMES[1] and currentTime > _HXUIPlus_DEV_DAMAGE_SET_TIMES[1][1] then
			_HXUIPlus_DEV_DEBUG_HP_PERCENT_PERSISTENT = _HXUIPlus_DEV_DAMAGE_SET_TIMES[1][2];

			table.remove(_HXUIPlus_DEV_DAMAGE_SET_TIMES, 1);
		end

		if #_HXUIPlus_DEV_DAMAGE_SET_TIMES == 0 then
			...
			for i = 1, totalDamageInstances do
				...
				if i < totalDamageInstances then
					...
					previousHitTime = previousHitTime + _HXUIPlus_DEV_DEBUG_INTERPOLATION_DELAY;
				end

				_HXUIPlus_DEV_DAMAGE_SET_TIMES[i] = {previousHitTime, previousHp};
			end
		end

		hppPercent = _HXUIPlus_DEV_DEBUG_HP_PERCENT_PERSISTENT;
	end
    ...
    if (imgui.Begin('TargetBar', true, windowFlags)) then
        ...
        local distValue = math.sqrt(targetEntity.Distance);
        local dist  = ('%.1f'):fmt(distValue);
		local targetNameText = targetEntity.Name;
		local targetHpPercent = targetEntity.HPPercent..'%';

        local isOor = (distValue > 21.8);
        ...
        if (gConfig.targetBarDarkenHpOutOfRange and isOor) then
            hpGradientStart = '#151515';
            hpGradientEnd   = '#1f1f1f';
        end
