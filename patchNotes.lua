...
local function InitializeTextures()
	if (HXUIPlusTexture == nil) then
		HXUIPlusTexture = LoadTexture("patchNotes/HXUIPlus");
	end
...
patchNotes.DrawWindow = function()
	...
	if (HXUIPlusTexture == nil or PatchVerTexture == nil or NewTexture == nil) then
		InitializeTextures();
	end
	if (HXUIPlusTexture == nil or PatchVerTexture == nil or NewTexture == nil) then
		return;
    ...
    if (gShowPatchNotes[1] and imgui.Begin('HXUIPlus PatchNotes', gShowPatchNotes, bit.bor(ImGuiWindowFlags_NoSavedSettings))) then
		imgui.Image(tonumber(ffi.cast("uint32_t", HXUIPlusTexture.image)), { 83, 53});
		imgui.SameLine()
