...
local function load_status_icon_from_theme(theme, status_id)
    -- Ensure valid status_id (should be between 0 and 0x3FF)
    if (status_id == nil or status_id < 0 or status_id > 0x3FF) then
        return nil;
    end
    ...
    -- Check different image file formats for the icon
    T{'.png', '.jpg', '.jpeg', '.bmp'}:forieach(function(ext, _)
        ...
        if (handle ~= nil) then
            ...
            icon_path = nil;  -- If not found, reset the path
        end
    end);

    -- If no icon found in the theme, fallback to internal resources
    if (icon_path == nil) then
        return load_status_icon_from_resource(status_id);
    end

    -- If an icon is found, load it using DirectX
    local dx_texture_ptr = ffi.new('IDirect3DTexture8*[1]');
    if (supports_alpha) then
        -- Handle alpha transparency (e.g., for PNG files)
        if (ffi.C.D3DXCreateTextureFromFileA(d3d8_device, icon_path, dx_texture_ptr) == ffi.C.S_OK) then
            return d3d8.gc_safe_release(ffi.cast('IDirect3DTexture8*', dx_texture_ptr[0]));
        end
    else
        -- Handle non-alpha images (e.g., BMP, JPG) using color-key transparency
    ...
    -- If we fail to load the icon, fall back to the dummy icon
    return load_dummy_icon();
end
...
statusHandler.get_job_theme_paths = function()
    local path = ('%s\\addons\\%s\\assets\\jobs\\'):fmt(AshitaCore:GetInstallPath(), 'HXUIPlus');
...
statusHandler.get_status_theme_paths = function()
    local path = ('%s\\addons\\%s\\assets\\status\\'):fmt(AshitaCore:GetInstallPath(), 'HXUIPlus');
    local directories = ashita.fs.get_directory(path);
    if (directories ~= nil) then
...
statusHandler.get_background_paths = function()
    local path = ('%s\\addons\\%s\\assets\\backgrounds\\'):fmt(AshitaCore:GetInstallPath(), 'HXUIPlus');
...
statusHandler.get_cursor_paths = function()
    local path = ('%s\\addons\\%s\\assets\\cursors\\'):fmt(AshitaCore:GetInstallPath(), 'HXUIPlus');
    local directories = ashita.fs.get_dir(path, '.*.png', true);
    if (directories ~= nil) then
        return directories;
    end
